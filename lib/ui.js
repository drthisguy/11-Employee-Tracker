...
return inquirer.prompt(
    ...
    choices: ["First Name", "Last Name", "Department", "Role", "Quit editor"]
    }
)
}
...
async getNewRole() {
    ...
    [ role ] = response.filter(a => a.title === choice.role);
     ...
     //capitalize each word
    choice.name = choice.role.replace(/\b[a-z]/g, char => char.toUpperCase());
    return role.id;
}
...
async getNewRoleName() {
    ...
    let input = await inquirer.prompt(
        ...
        type: "input",
        name: "name",
        message: "Enter a new name for this position: "
        }
    )  
    //capitalize each word
    input.name = input.name.replace(/\b[a-z]/g, char => char.toUpperCase());
