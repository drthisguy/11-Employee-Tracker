...
addRole(role) {
    ...
    err => {if (err) throw err})
    console.log(`\nThe new job title ${role} has been added sucessfully!\n`);
}
...
joinByDepts(dept) {
    ...
    (err, res) => {
        ...
        console.clear();
        const [{ name }] = res;
        console.log(console.table(`All employees from ${name}`, res));
    }
  )
}
...
joinRolesToDepts(dept) {
    ...
    (err, res) => {
        ...
        console.log(`${console.table(`${name} Department:`, res)}`);
        console.log(`\nThe payroll budget for ${name} is: ${budget}`);
    }
  )
}
}