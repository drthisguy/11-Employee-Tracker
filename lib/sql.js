...
addDept(department) {
    ...
    err => {if (err) throw err})
    console.log(`\nNew department ${department.name} added sucessfully!\n`);
}
...
updateDeptName(update) {
    ...
    [{ name: update.name }, { id: update.id }],
    
    err => { if (err) throw err})
    console.log(`\nDepartmen # ${update.id} has been changed to: "${update.name}" sucessfully!`);
}
...
rmDept(remove) {
    ...
    console.log(`\nThe department ${remove.name} has been removed from the company database.`),
    err => { if (err) throw err});
}
...
joinByDepts(dept) {
    ...
    (err, res) => {
        ...
        console.clear();
        const [{ name }] = res;
        console.log(`\n${console.table(`All employees from ${name}:`, res)}`);
    }
  )
}

joinEmpToDepts() {
    connection.query(`SELECT * FROM employees LEFT JOIN departments USING (id) WHERE (employees.dept_id ? = department.id ?)`,
    (err, res) => {
        if (err) console.log(err);
