...
addRole(role) {
    ...
    console.log(`\nNew role: "${role.name}" has been created sucessfully!\n`);
    resolve();
    })
    })
}
...
updateDeptName(update) {
    ...
    err => { if (err) throw err})
    console.log(`\nDepartment # ${update.id} has been changed to: "${update.department}" sucessfully!\n`);
}
...
rmRole(remove) { 
    ...
    err => { 
        ...
        {console.log('\nYou currently have employees assigned to this position.  Please remove them before removal\n');
