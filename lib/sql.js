...
updateRoleName(update) {
    ...
    console.log(`\nRole # ${colors.fg.green}${update.id} ${colors.sp.reset}has been changed to: "${colors.fg.green}${update.department}" ${colors.sp.reset}sucessfully!\n`);
}
...
joinRolesToDepts(dept) {
    ...
    (err, res) => {
        ...
        console.table(`${department} Department:`, res)
...
joinEmploysToRoles(role) {
    ...
    (err, res) => {
        ...
        console.table(`${title} Position:`, res);
        console.log(`\nThe payroll budget for ${colors.fg.blue}${title}${colors.sp.reset} is: ${colors.fg.green}${budget}.00${colors.sp.reset}`);
    }
  )
}
}